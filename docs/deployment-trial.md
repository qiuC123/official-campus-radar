...
后续用户已确认部署仅本人可访问的内部预览；其独立边界见文末。此前的基础环境准备记录不代表公网已发布。

2026-09-06 后续进展：云端官网采集、12:00/20:00 定时任务及同盘版本恢复已完成，最新事实与入口见 `cloud-update.md`。下文“未安装浏览器/未启用更新”等为 2026-09-05 阶段记录，不再代表当前状态；SSH 私有访问边界保持不变。
...
- `export_public_snapshot --out <新的绝对路径>` 从原有公告门控后的公开展示模型生成新 SQLite，只含 `snapshot_meta`、`public_batch` 两张表。使用显式字段清单，不复制业务库再删表；不导出个人进度、账号、会话、采集配置、错误日志或内部审计。原库事务启用 `query_only`，输出不得覆盖已有文件。
- 这是展示快照，不是采集库；快照内已准入的岗位保留筛选、分页、历史和详情。服务器不重新放宽准入、不执行采集，后续更新须在原环境重新核验再导出。页面明确提示未启用云端自动更新。
- 单独的 `settings_private_preview` 必须显式 `RADAR_SSH_PREVIEW=1`，只允许 loopback Host；关闭 HTTPS 跳转只限该私有进程。浏览器到本机 `127.0.0.1:18765` 是 HTTP，跨公网由 SSH 加密，到服务器 `127.0.0.1:8765` 后由只读 Gunicorn 处理。正式 `settings_public` 仍强制 HTTPS，不得把私有预览直接改绑公网。
- 静态文件使用 Django 静态处理器，仅用于本轮私有预览；正式公网须独立静态服务器/反向代理。预览也保持 DEBUG=False，后台和个人接口为 404。
- `setup_private_preview.py` 仅允许在专用目录首次安装，拒绝覆盖已有服务/账号/快照；以无登录权限的 `radar-preview` 用户运行，数据库 root 所有、服务组只读，密钥环境文件 root-only。systemd 服务监听回环、一个 worker、512 MiB 内存上限。
- 新建的 `radar-tunnel` 只允许本地转发至预览端口，不能登录 shell、反向转发或代理转发；SSH 配置先备份、验证后 reload。专用私钥只保留本机，云端仅登记公钥；主机公钥经已有 Workbench 连接读取后固定验证，禁止关闭 host key 校验。
- 运行 `scripts/open_private_preview.ps1` 建立隧道；关闭隧道后本机预览入口失效。服务器服务可随机器重启恢复，但不包含自动更新任务。无需域名，不开放网站端口，不修改安全组。

### 私有预览验收 — 2026-09-05

- 快照生成于北京时间 16:27:57；17 家企业、33 个在招批次、9422 个当前岗位、2 个历史批次。SHA-256：`68d9d0af05adcb54ee8a4126f2f091b510f1da9f37e18c978f33f4971a0a0942`。本地原库不变，没有应用任何业务迁移。
- Windows 完整 622 项测试通过（43.146 秒），阿里云完整 622 项通过（54.454 秒）。新增独立 WSGI 启动测试，避免只测 Django Client 而漏掉启动顺序错误。
- 初次同步修复文件时 Workbench 拒绝覆盖，旧 WSGI 的初始化顺序造成过 4 次启动重试；已停止该服务、显式更新本轮拥有的两个文件、重跑启动测试并重启。验收以修复后真实 HTTP 和 systemd 状态为准，不把安装脚本返回成功等同于服务正常。
- 首页、历史、公司筛选、分页、岗位详情、静态 CSS 为 200；后台、个人页面、`.env` 和数据库路径为 404；POST 首页为 405；未知 Host 为 400。浏览器实际确认 vivo 4 个独立项目正常展示，首页无个人进度控件。
- 服务器服务为 `radar-preview` 用户、active/running，实测占用约 83 MiB（空闲快照，不是压测）；仅 SSH 22 对外监听，网站为 `127.0.0.1:8765`。数据库权限为 `440 root:radar-preview`，环境文件 `600 root:root`，服务用户不能写数据库。
- `sshd -T` 确认隧道用户仅允许 local forwarding、`PermitOpen 127.0.0.1:8765`、`PermitListen none`、无密码/代理转发；真实 shell 请求被 nologin 拒绝。没有重启服务器、改安全组或执行真实官网采集。
- 隧道入口只在用户电脑可用；关闭当前 SSH 进程或重启电脑后，需要重新运行 `scripts/open_private_preview.ps1`。本轮未做开机自动连接，也未推送 GitHub。
