...
4. 记录低频、公开、无需登录的访问策略；
5. 使用已保存且不含敏感信息的 fixture 验证完整选择器配置；HTML 来源至少要有稳定公告/岗位 identity，以及标题、分类、对象、发布日期、截止日期、岗位名、地点、公告链接和原文 selector。缺一项都不能从 `verified` 进入 `enabled`；
6. 运行 `py -3.13 manage.py import_source_catalog --path data/source_catalog.csv --dry-run`；
7. 在启用新的线上来源批次前取得单独批准。
...
离线选择器验证完成且用户批准该批次后，才能再执行 `--to-state enabled`。暂停或撤销分别使用 `suspended`、`revoked`，不能直接编辑 `is_active`/`is_verified` 绕过事件链。外部 ATS/投递 host 必须先通过 `approve_application_host(...)` 本地服务记录官网入口证据；公告 URL 仍只允许来源自身 host，二者不会合并为一个宽松白名单。
