...
DEFAULT_OUTPUT = ROOT / "work" / "phase-02-t6-cycle-01.json"
...
def build_report(update_run_ids: list[int]) -> dict:
    ...
    if len(fetches) != 25 or len({fetch.source_id for fetch in fetches}) != 25:
        raise ValueError("Cycle 01 must contain exactly one attempt for each of 25 sources")
    events = list(
        PublicationEvent.objects.filter(
            source_version__fetch_run__update_run_id__in=unique_run_ids
        ).select_related("source_version__fetch_run", "source_version__source")
    )
    ...
    return {
        "cycle": "Phase 02 / T6 Cycle 01",
        "mode": "first controlled live collection",
        "update_run_ids": [run.pk for run in runs],
        "database_writes": True,
        "raw_responses_persisted": False,
        "source_attempt_count": len(results),
        "network_completed_source_count": sum(
            result["fetch_status"] != FetchRun.Status.FAILED for result in results
        ),
        ...
        "run_statuses": [
            {"update_run_id": run.pk, "status": run.status} for run in runs
        ],
        "results": results,
        "completed": True,
...
def main() -> int:
    ...
    if args.check:
        ...
        if existing != rendered:
            raise SystemExit("T6 Cycle 01 report is stale")
    else:
