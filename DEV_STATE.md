...
- 当前共有 39 个招聘批次：23 个 `admitted`、4 个 `superseded`、12 个 `excluded`、0 个 `pending`。正式页只显示 13 家企业的 23 个已准入批次和 5719 个当前岗位。
...
- 2026-08-31 完整回归共 501 项测试通过；`manage.py check`、迁移检查、目录确定性检查和 `git diff --check` 均通过。正式首页、历史页和“我的进度”页均返回 HTTP 200。
...
- 公告与投递入口复核后，理想汽车 3 个批次的公告统一改为官网“招聘动态”页，美的 3 个批次改为明确列出博士/应届生/实习生对象的校园招聘首页，腾讯 3 个实习批次改为展示招聘项目与校招公告的腾讯校招首页；各批次投递链接仍保留直达岗位筛选页。博世未发现独立的当前 2027 公告，继续按兜底规则复用官网工作机会页。
