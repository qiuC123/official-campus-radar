...
- Cycle 02 本阶段在 23:37 运行 `py -3.13 manage.py test -v 1`：255/256 通过；旧健康状态测试因 22:00 后“计划更新漏跑”优先级而失败。该测试未固定时间，本 Cycle 未改动相关生产代码或测试，不能把本次完整套写成全通过。
...
为北森、大易经典版、大易新版、智联定制站分别形成只读解析草案，并用拼多多、理想汽车作为 JSON/API 对照。每个家族仍需逐家核对校招范围、岗位、地点和分页；不得把共享页面架构直接写成来源准入成功，也不得提前进入 T4。
