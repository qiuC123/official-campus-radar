...
Phase and status: Phase 02 / H1、H2、G10、H3 已通过。T3 Cycle 03 已完成拼多多、理想汽车、中国中车的两页 API 复验；累计 3/50 家可稳定接入，仍未达到 25/50 最低线，T4 禁止启动。
...
- 四个家族样板已有机器可读解析草案和 7 项离线测试。Cycle 03 已确认中国中车的 form POST 分页正文，最简请求可连续取得两页不同校招岗位；它现在满足 H3 五项条件，稳定接入数更新为 3/50。当前生产 JSON 适配器仍不支持 form POST，理想汽车 `total_pages` 也不能当作岗位总数，这两项属于正式接入前的适配器工作，不影响本轮接口可用性结论。
...
等待确认下一步：可以先为 JSON 适配器增加 form POST 和“总页数”分页语义，也可以继续处理北森多页、大易经典版校招范围与智联公开数据接口。无论选择哪项，都不得把 T3 结果写成 T4 来源准入成功。
