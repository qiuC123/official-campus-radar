...
除 ADR 0004 的中国联通岗位采集、受控官网公告核验和 ADR 0010 的按来源可投递状态探测外，不扩大生产浏览器采集。自动备份尚未开始；云部署仅进入试运行准备，未开放公网服务。招聘雷达不再执行微信公众号搜索、回读或导入。

## Exact next task

进入阿里云 ECS 小规模试部署：用户已购买 2 核 2 GiB Alibaba Cloud Linux 3，并授权准备代码与独立运行环境。公开只读模式、SQLite 只读连接及个人/管理隔离边界见 `docs/deployment-trial.md`；低预算试运行先使用 SQLite，不在 2 GiB 上同时引入 PostgreSQL。先验证独立 Python 3.13 环境和代码，随后再验收浏览器依赖、数据部署、域名/HTTPS、反向代理、备份和云端定时任务。不得直接把当前本地 SQLite、调试服务或 Windows 计划任务暴露到公网。Exa-first 迁移 `0035` 继续不应用，生产采集仍只使用已准入官网和官方 ATS。
