...
进入公网部署准备：先冻结公开只读页面与管理/采集面的隔离边界，再选定托管环境、PostgreSQL、密钥注入、备份和云端定时任务；不得直接把当前本地 SQLite、调试服务或 Windows 计划任务暴露到公网。Exa-first 迁移 `0035` 继续不应用，生产采集仍只使用已准入官网和官方 ATS。
