...
- 修复后的单源只读复验为 HTTP 200、16 个唯一原始岗位、无重复冲突，但当前接口的 `projectName` 已全部切换为 `2027校园招聘`，数据库配置仍严格筛选 `2026校园招聘`，因此提取结果为 0。全量验收结论仍为 28/29，不能把接口可访问误报成业务通过；下一步需经确认后更新并重新准入一汽-大众 2027 届来源配置。
