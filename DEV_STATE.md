...
Phase and status: Phase 02 / H1 Cycle 01/02 收到修改意见，静态原型进入 Cycle 03；H1/H2 未确认，因此 G07～G10 不得记为正式接受；H3 尚未开始。
...
- H1：用户确认 Cycle 03 高密度横向表格、代表岗位单元格和批次行进度的位置与含义。
...
先等待用户验收 H1 Cycle 03；若仍有修改意见，新建 Cycle 04。H1 通过后先明确批次行进度的领域含义，再同步 Django 预览页并进行 H2。全程不自动访问任何企业网站。
