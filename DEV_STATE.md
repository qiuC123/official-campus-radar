...
除 ADR 0004 的中国联通岗位采集、受控官网公告核验和 ADR 0010 的按来源可投递状态探测外，不扩大生产浏览器采集。自动备份尚未开始；云端仅提供 SSH 私有预览，未开放公网服务。招聘雷达不再执行微信公众号搜索、回读或导入。

## Exact next task

阿里云 SSH 私有预览已完成：独立 Python 3.13、展示快照与非 root 只读服务已部署，通过本机 `http://127.0.0.1:18765/` 隧道访问。快照为 17 家企业、33 个在招批次和 2 个历史批次，不含个人进度、账号或采集配置，尚未启用云端自动更新。公开只读模式、SQLite 只读连接及个人/管理隔离边界见 `docs/deployment-trial.md`；低预算试运行先使用 SQLite，不在 2 GiB 上同时引入 PostgreSQL。下一阶段单独验收浏览器依赖、数据更新方案、域名/HTTPS、反向代理、备份和云端定时任务，不把内部 HTTP 预览直接改绑公网。Exa-first 迁移 `0035` 继续不应用，生产采集仍只使用已准入官网和官方 ATS。
