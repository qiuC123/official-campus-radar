...
后续 MCP 诊断修复已离线验收：主任务独立运行15项新错误回归通过，并用全新stdio的零额度调用核对新增 `error_facts`/`artifacts.errors` 及持久化摘要一致。维护任务全量119项Python、5项TypeScript及类型检查通过。原失败摘要哈希与上一批记录完全一致，旧错误文件仍空；历史HTTP状态不可追溯。机器证据见 `work/xiaomi-mcp-error-diagnostics-verification-20260907.json`。修复不改入口/预算/UA/TLS/代理，也未再次访问小米；下一次获授权的调用前需刷新已启动MCP进程。

用户再次要求继续项目后，本批通过普通 HTTP 取得小米校园、新零售和实习官方公告原文及招聘对象，并核对官网数字公告 ID 的详情渲染逻辑。实际 5 次官网请求、62.119 秒、全部 HTTP 200，未请求 ATS 或重跑实习；ATS 台账仍为 257/400、岗位已知 10/12。7 条公告/FAQ 与 3 个官网导航项目分别保留，不当成正式批次数量；官网另指向海外岗位入口，尚未访问。原文已冻结，但公告到 ATS 项目的绑定、全量覆盖与独立可投仍待核验。当前已连接 MCP 的零额度结果仍缺诊断字段，需要重连后才采用新诊断。最新记录见 `docs/handoffs/xiaomi-official-announcements-20260907.md`，离线重建工具为 `tools/prepare_xiaomi_announcements.py`；未改数据库、云端或生产准入。
