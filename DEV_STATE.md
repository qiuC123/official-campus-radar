...
Phase and status: Phase 02 / H1、H2、G10、H3 已通过。T3 Cycle 02 已按用户确认启动；当前完成平台指纹基线和样板页只读验证，家族适配验证尚未完成。T3 仍未达到 25/50 最低线，T4 禁止启动。
...
- 四个家族样板已有机器可读解析草案和 7 项离线测试。中国中车发现可最简头重放的新版大易岗位 API，但未捕获页码参数；广汽丰田、信通院可读服务端 HTML；中国联通只能从渲染后 DOM 读取。由于分页、招聘范围或生产 transport 仍不完整，稳定接入数仍为 2/50。
...
继续处理四类草案的阻断项：北森验证多页规则，大易经典版证明 `changePage` 的只读分页请求并拆分校招范围，大易新版确认页码参数，智联优先寻找公开数据接口而不是引入生产浏览器。不得把草案写成来源准入成功，也不得提前进入 T4。
