...
2026-09-07 用户已选择优先扩容企业，并批准华为、阿里巴巴、字节跳动、小米、中国移动第一批实施。真实复验后新增正式接入 0 家，本地仍为 25 家企业、33 个来源，未改云端。小米现行官网已找到可匿名读取的公告接口和新 ATS，但岗位请求带签名；其余企业分别存在会话依赖、签名、未找到岗位接口或本机 TLS 兼容问题。逐家证据、执行偏差及下一步见 `docs/handoffs/company-expansion-20260907.md`。下一待确认事项是小米专项受控浏览器验证；没有扩大 ADR 0004 的生产浏览器范围。开发探测工具已补充 CSRF 脱敏和访问限制立即停止，完整测试 637 项（2 项平台跳过）。
