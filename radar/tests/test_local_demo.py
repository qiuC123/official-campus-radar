...
from radar.models import ApplicationProgress, Organization, RecruitmentNotice, UpdateRun
...
class LocalDemoCommandTests(TestCase):
    def test_load_and_remove_controlled_local_demo(self) -> None:
        ...
        self.assertEqual(RecruitmentNotice.objects.filter(organization=organization).count(), 3)
        self.assertTrue(ApplicationProgress.objects.filter(notice__organization=organization, status="interviewed").exists())
        ...
        self.assertTrue(RecruitmentNotice.objects.formal().filter(organization=organization, status="active").exists())
