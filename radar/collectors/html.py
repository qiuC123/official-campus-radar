...
class HtmlSourceAdapter:
    ...
    def extract(self, source: OfficialSource, page: FetchedPage) -> list[NoticeCandidate]:
        if page.not_modified:
            return []
        config = source.parser_config
        required = ("notice_selector", "notice_url_selector", "title_selector", "recruitment_type_selector")
        if not isinstance(config, dict) or any(not config.get(key) for key in required):
            raise ValueError("selector configuration requires notice, URL, title, and recruitment type selectors")
        ...
        for index, node in enumerate(soup.select(config["notice_selector"]), start=1):
            ...
            if not title_node or not notice_link or not notice_link.get("href") or not recruitment_type_node:
                continue
            def selected(key):
                return node.select_one(config[key]) if config.get(key) else None

            def text_value(key):
                selected_node = selected(key)
                return selected_node.get_text(" ", strip=True) if selected_node else ""

            deadline_text = text_value("deadline_selector")
            try:
                deadline = date.fromisoformat(deadline_text) if deadline_text else None
            except ValueError:
                deadline = None
            published_text = text_value("published_on_selector")
            try:
                published_on = date.fromisoformat(published_text) if published_text else None
            except ValueError:
                published_on = None
            app_node = node.select_one(config["application_selector"]) if config.get("application_selector") else None
            application_href = app_node.get("href") if app_node else None
            app_url = urljoin(page.canonical_url, application_href) if application_href else None
            location = text_value("location_selector")
            ...
            identity_key = (
                ...
                else text_value("notice_id_selector") if identity_selector else ""
            )
            position_attribute = config.get("position_id_attribute")
            position_key = (
                str(node.get(position_attribute, "")).strip()
                if position_attribute
                else text_value("position_id_selector")
            )
            locator = (
            ...
            withdrawn = bool(config.get("withdrawn_selector") and node.select_one(config["withdrawn_selector"]))
            target_audience = text_value("target_audience_selector")
            position_title_node = selected("position_title_selector")
            position_title = (
            ...
            if identity_key:
                ...
                for field_name, selector_key, raw_value, parsed_value in optional_notice_fields:
                    if selected(selector_key) is not None:
            ...
            if identity_key and position_key and position_title_node is not None:
                position_evidence["position_title"] = FieldEvidenceValue(
                    position_title,
                    f"{locator} {config['position_title_selector']}",
                    position_title,
                )
            if identity_key and position_key and selected("location_selector") is not None:
                position_evidence["location"] = FieldEvidenceValue(
                    location,
                    f"{locator} {config['location_selector']}",
                    location,
                )
            if identity_key and position_key and application_href:
                position_evidence["application_link"] = FieldEvidenceValue(
                    application_href,
                    f"{locator} {config.get('application_selector', '')}@href".strip(),
                    app_url,
                )
            candidates.append(NoticeCandidate(
                title=title_node.get_text(" ", strip=True), official_notice_url=notice_url,
                recruitment_type=recruitment_type_text, target_audience=target_audience, published_on=published_on, deadline=deadline,
                withdrawn=withdrawn, evidence_excerpt=text_value("excerpt_selector"),
                positions=(PositionCandidate(position_title, location, node.get_text(" ", strip=True), app_url, locator, f"{locator} {config.get('application_selector', '')}".strip(), position_key, position_evidence),),
                field_locators={"title": f"{locator} {config['title_selector']}", "recruitment_type": f"{locator} {config['recruitment_type_selector']}", "notice_url": f"{locator} {config['notice_url_selector']}", "deadline": f"{locator} {config.get('deadline_selector', '')}".strip()},
                identity_key=identity_key,
                field_evidence=field_evidence,
                positions_complete=bool(config.get("positions_complete", False)),
            ))
        return candidates
