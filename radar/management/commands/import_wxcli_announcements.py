...
from radar.services.announcements import import_wxcli_announcement
...
class Command(BaseCommand):
    help = "将 wxcli Candidate Batch 回读为微信证据并导入公告；不会自动创建正式批次。"

    def add_arguments(self, parser):
        parser.add_argument("--organization", required=True)
        parser.add_argument("--input", required=True)
        parser.add_argument("--wxcli-path", default="wxcli")
        parser.add_argument(
            "--allow-browser",
            action="store_true",
            help="显式授权 wxcli 打开其独立可见 Chrome；Candidate Batch 本身不能授权。",
        )
    ...
    def handle(self, *args, **options):
        try:
            ...
            result = WxCliClient(options["wxcli_path"]).hydrate_candidate_batch(
                payload,
                allow_browser=options["allow_browser"],
            )
            announcements = [
                import_wxcli_announcement(organization, candidate)
                for candidate in result.verified_candidates
            ]
        except Organization.DoesNotExist as error:
            raise CommandError("organization not found") from error
        except (OSError, json.JSONDecodeError, WxCliError, ValidationError) as error:
